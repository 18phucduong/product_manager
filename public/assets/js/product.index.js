...
function setPaginateEvent() {
  let paginateButtonsQuery =
    document.ajaxInformation.pagination.paginateItemsSelector;
  ...
  paginateButtons.forEach((paginationItem) => {
    paginationItem.onclick = function () {
      currentPage = document.ajaxInformation.pagination.page =
        paginationItem.getAttribute("data-page");
      loadView();
    };
  });
}
...
function loadAjax(setting) {
  ...
  contentType = setting.hasOwnProperty("contentType")
    ? setting.contentType
